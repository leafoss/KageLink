...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Kage Pilot v0.3f Dojo loop: find trainer -> click OK -> fight -> recover -> repeat / "
            "buscar treinador -> clicar OK -> lutar -> recuperar -> repetir"
        )
    )
...
def _run_round(args, *, round_number: int) -> bool:
    ...
    for line in process.stdout:
        print(line, end="")
        if "result=ready" in line.casefold():
            ready = True
...
def main() -> int:
    ...
    print("Kage Pilot v0.3f FULL DOJO LOOP")
    print("FIND TRAINER -> CLICK SPRITE -> WAIT 10s -> CLICK DIALOG OK -> WAIT 5s")
    print("COMBAT -> VICTORY CHAT -> RETURN/SEARCH -> RECOVER -> REPEAT")
