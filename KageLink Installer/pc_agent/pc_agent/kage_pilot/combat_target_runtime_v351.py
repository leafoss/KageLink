...
def install_combat_target_bridge(runtime: Any):
    """Install PR23 persistent logical combat targeting over the validated chain."""
        ...
        def process(self, frame_bgr, *, timestamp=None):
            ...
            if memory.active:
                ...
                if current is not None:
                    context, metrics = self._context_and_metrics(current)
                    if metrics is not None and str(
                        getattr(context, "state", "")
                    ).upper() in {"VISIBLE", "OCCLUDED"}:
                        chosen = current
                if chosen is None:
    ...
    class PersistentCombatDecisionEngine(BaseEngine):
        def decide(
            ...
            *,
            ...
            face = snapshot.last_contact_direction
            navigation = str(getattr(base, "navigation", "HOLD"))
            mode = str(getattr(base, "mode", "ENGAGED_SEARCH"))
            h_opportunity = bool(getattr(base, "h_opportunity", False))
            reason = str(getattr(base, "reason", ""))

            if snapshot.active and state.target is None:
                h_opportunity = False
                if snapshot.movement_mode == "MELEE_LOCK":
                    mode = "MELEE"
                    navigation = (
                        f"FACE_{face}" if face in _CARDINAL else "HOLD_MELEE"
                    )
                elif (
                    0.60 <= snapshot.time_since_last_seen < 1.50
                    and face in _CARDINAL
                ):
                    mode = "RECOVER"
                    navigation = f"MOVE_{face}"
                else:
                    mode = "MEMORY_HOLD"
                    navigation = "HOLD"
                reason = (
                    f"persistent combat target #{snapshot.combat_target_id}; "
                    f"state={snapshot.target_state}; visual track temporarily absent; "
                    "local rebind before global search"
                )
            elif snapshot.active:
                if (
                    snapshot.movement_mode == "MELEE_LOCK"
                    and navigation.startswith("MOVE_")
                ):
                    navigation = (
                        f"FACE_{face}" if face in _CARDINAL else "HOLD_MELEE"
                    )
                    mode = "MELEE"
                elif face in _CARDINAL and (
                    navigation.startswith("FACE_")
                    or navigation.startswith("MOVE_")
                ):
                    prefix = "FACE" if navigation.startswith("FACE_") else "MOVE"
                    navigation = f"{prefix}_{face}"
                reason += (
            ...
            return _decision_from_base(
                ...
                face=(
                    face
                    if face in _CARDINAL
                    else str(getattr(base, "face", "-"))
                ),
    ...
    class StableCombatControlPlanner(BasePlanner):
        def plan(
            ...
            *,
            ...
            if (
                getattr(decision, "movement_mode", "") == "MELEE_LOCK"
                and command.move_pulse
            ):
                command = replace(
                    command,
                    move_pulse=None,
                    ...
                    reason=(
                        str(command.reason)
                        + "; pursuit cancelled inside contact radius"
                    ),
                )
            if getattr(decision, "target_state", "") == CombatTargetState.LOST.value:
                command = replace(command, move_pulse=None, face_pulse=None)
            if logical_id is not None:
                parts = []
