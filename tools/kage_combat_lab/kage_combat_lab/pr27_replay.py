...
_FRAME_PATTERN = re.compile(
    r"frame_(\d+)_.*_(FULL|ROI)\.(?:png|jpg|jpeg)$",
    re.IGNORECASE,
)
...
def _legacy_trainer_ghost_bbox(
    full_paths: list[Path],
) -> tuple[NativeRect | None, float]:
    if len(full_paths) < 4:
        return None, 0.0
    sampled = full_paths[:: max(1, len(full_paths) // 12)][:12]
    frames = [_read_image(path) for path in sampled]
    if any(
        frame.shape[:2] != (_DEBUG_ARENA_RECT.height, _DEBUG_ARENA_RECT.width)
        for frame in frames
    ):
        return None, 0.0
    gray_stack = np.stack(
    ...
    for label in range(1, count):
        ...
        edge_density = np.count_nonzero(
            edges[y : y + height, x : x + width]
        ) / max(1, width * height)
        confidence = min(1.0, edge_density * 5.0) * max(
            0.0,
            1.0 - float(np.mean(patch_variance)) / 3.0,
        )
...
def _audit_frame_24_roi(roi_paths: list[Path]) -> dict[str, object]:
    path = next(
        (item for item in roi_paths if item.name.startswith("frame_000024_")),
        None,
    )
    if path is None:
        return {
        ...
        for right, right_top, right_bottom in vertical:
            ...
            for top, top_left, top_right in horizontal:
                for bottom, bottom_left, bottom_right in horizontal:
                    height = bottom - top
                    if not 50 <= height <= 82 or top > 120 or left > 260:
                        continue
                    coverage = 0.0
...
def _save_result(output: Path, source_name: str, result) -> None:
    ...
    roi = result.overlay_bgr[
        fixed.top - 4 * 64 : fixed.bottom + 4 * 64,
        fixed.left - 4 * 64 : fixed.right + 4 * 64,
    ]
    cv2.imwrite(str(directories["roi"] / f"{stem}.png"), roi)
    cv2.imwrite(
        str(directories["raw_difference"] / f"{stem}.png"),
        result.raw_difference_mask,
    )
    cv2.imwrite(
        str(directories["compensated_residual"] / f"{stem}.png"),
        result.compensated_residual_mask,
    )
    cv2.imwrite(
        str(directories["animated_background"] / f"{stem}.png"),
        result.animated_background_mask,
    )
...
def run_replay(
    ...
    *,
    ...
    try:
        ...
        self_confirmed = self_lost = alignment_failures = 0
        water_false_positives = int(bool(frame_24_audit.get("passes_pr278_body_core")))
        trainer_ghost_detections = int(trainer_bbox is not None)
        entity_candidates = humanoid_candidates = hostile_confirmations = 0
        planned_movements_blocked = role_conflicts = 0
        ...
        with jsonl_path.open("w", encoding="utf-8") as log:
            for path in full_paths:
                ...
                alignment_failures += int(
                    result.fixed_self_bbox != system.grid.self_cell_rect()
                )
                ...
                planned_movements_blocked += int(result.planned_action == "NONE")
                role_conflicts += int(
                    result.action_block_reason == "SELF_NOT_CONFIRMED"
                    and result.planned_action != "NONE"
                )
        frame_12 = next(
            (
                item
                for item in frame_records
                if item["source_file"].startswith("frame_000012_")
            ),
            None,
        )
        frame_116_120 = [
            ...
            for item in frame_records
            if any(
                item["source_file"].startswith(f"frame_{index:06d}_")
        ...
        frame_12_fixed = bool(
            ...
            and (frame_12["self_body_found"] or frame_12["planned_action"] == "NONE")
        )
        summary = {
            ...
            "frame_24_is_roi_only": any(
                path.name.startswith("frame_000024_") for path in roi_paths
            ),
            ...
            "frame_12_fixed_grid_ok": frame_12_fixed,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.trainer_bbox_output:
            values = tuple(
                int(value.strip()) for value in args.trainer_bbox_output.split(",")
            )
            if len(values) != 4:
