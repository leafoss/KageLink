...
def run_dojo(argv: list[str]) -> int:
    ...
    print(f"Recovery / Recuperacao: HP>={config.recovery_hp_percent:.0f}% Chakra>={config.recovery_chakra_percent:.0f}%")
    ...
    print(f"DOJO_FINAL phase={snapshot.phase.value} completed={snapshot.completed_rounds} return_code={snapshot.return_code}")
