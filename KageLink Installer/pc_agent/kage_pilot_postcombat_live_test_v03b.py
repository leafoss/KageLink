...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Kage Pilot v0.3b isolated post-combat live test: leader navigation + V toggle / "
            "teste isolado: navegar ate o lider + alternar V"
        )
    )
...
def main() -> int:
    ...
    print("Kage Pilot v0.3b POST-COMBAT LIVE TEST")
    print("NO R / NO H - DEAD-MAN ARROWS + V TOGGLE ONLY")
    print("SEM R / SEM H - SOMENTE PULSOS DE SETA + TOGGLE V")
    ...
    try:
        ...
        while time.monotonic() - started < timeout:
            ...
            # Absolute safety invariant: post-combat has no persistent key state.
            ...
            if decision.state == "READY":
                ready = True
                print("POST_COMBAT_TEST_OK / TESTE_POS_COMBATE_OK")
                break
        ...
        print(f"POST-COMBAT TEST STOPPED / TESTE INTERROMPIDO: {type(exc).__name__}: {exc}")
    finally:
        controller.release_all()
        source.close()

    if not ready:
        state_name = getattr(engine, "state", "UNKNOWN")
        print(f"POST_COMBAT_TEST_INCOMPLETE state={state_name} / TESTE_INCOMPLETO estado={state_name}")
