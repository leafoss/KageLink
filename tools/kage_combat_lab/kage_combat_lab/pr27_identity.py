...
def lock_track_role(
    ...
    *,
    ...
    if track.role_locked and current is not desired:
        track.role_conflict_reason = f"ROLE_FLIP_BLOCKED:{current.value}->{desired.value}"
        return False
    track.role = desired
    track.role_locked = desired in {SpriteRole.SELF, SpriteRole.ENEMY, SpriteRole.NPC}
    track.role_assigned_frame = frame_index if track.role_assigned_frame < 0 else track.role_assigned_frame
    track.role_source = source
    track.role_confidence = max(track.role_confidence, float(confidence))
...
class SelfIdentityCapsule:
    """Slow-changing SELF identity model; ambiguous frames never update it."""
    ...
    descriptors: deque[AppearanceDescriptor] = field(default_factory=lambda: deque(maxlen=12))
    update_count: int = 0
    ...
    def score(self, observation: SpriteObservation) -> tuple[float, float, float]:
        ...
        if median is None:
            ...
            mw, mh = median
            width_ratio = min(width, mw) / max(1.0, max(width, mw))
            height_ratio = min(height, mh) / max(1.0, max(height, mh))
            size_score = math.sqrt(max(0.0, width_ratio * height_ratio))
        if not self.descriptors:
            appearance_score = 1.0
        else:
            appearance_score = max(
                DescriptorFactory.similarity(reference, observation.descriptor)
    ...
    def update(self, observation: SpriteObservation, *, clean: bool) -> None:
        if not clean or observation.body_bbox is None:
            return
        _, _, width, height = observation.body_bbox
        self.widths.append(float(width))
        self.heights.append(float(height))
        self.descriptors.append(copy_descriptor(observation.descriptor))
        self.update_count += 1


class SelfTracker:
    """Dedicated first-stage tracker. SELF never competes with ENEMY/NPC tracks."""
    ...
    def _fallback_anchor(self, arena_shape: Sequence[int]) -> tuple[float, float]:
        return (
            int(arena_shape[1]) * self.config.player_anchor_x_ratio,
            int(arena_shape[0]) * self.config.player_anchor_y_ratio,
        )

    def _predicted_anchor(self, arena_shape: Sequence[int]) -> tuple[float, float]:
        if self.track is None:
            return self.expected_anchor or self._fallback_anchor(arena_shape)
        if len(self.track.movement_history) >= 2:
            previous, current = self.track.movement_history[-2], self.track.movement_history[-1]
    ...
    def _candidate_score(
        ...
        *,
        ...
        if not observation.has_body_lock or observation.body_anchor is None or observation.body_bbox is None:
            return 0.0, 0.0, {"rejection_reason": "SELF_BODY_LOCK_MISSING"}
        identity_score, appearance_score, size_score = self.identity.score(observation)
        distance = math.hypot(
            observation.body_anchor[0] - predicted[0],
            observation.body_anchor[1] - predicted[1],
        )
        max_distance = self.config.player_anchor_radius_px if first_lock else max(48.0, self.config.player_anchor_radius_px * 1.25)
        ...
        if not first_lock and self.identity.ready:
            if identity_score < self.config.self_identity_min_score:
                return 0.0, identity_score, {
        ...
        proximity = max(0.0, 1.0 - distance / max(1.0, max_distance))
        overlap = bbox_iou(None if self.track is None else self.track.body_bbox, observation.body_bbox)
        body = max(0.0, min(1.0, observation.body_confidence))
        if first_lock:
            score = 0.62 * proximity + 0.18 * body + 0.12 * size_score + 0.08 * appearance_score
        else:
            score = 0.38 * proximity + 0.30 * identity_score + 0.17 * overlap + 0.10 * body + 0.05 * size_score
    ...
    @staticmethod
    def _new_track(observation: SpriteObservation, frame_index: int) -> TrackedSprite:
        track = TrackedSprite(
            ...
            appearance_signature=copy_descriptor(observation.descriptor),
            ...
            role_source="self_bootstrap",
            role_confidence=max(0.75, observation.body_confidence),
            track_source="calibrated" if frame_index == 0 else "reacquired",
        )
    ...
    def _update_track(
        ...
        *,
        ...
        self.track.appearance_signature = copy_descriptor(observation.descriptor)
        ...
        self.track.confidence = max(self.track.confidence * 0.75, association_score)
        ...
        if observation.body_anchor is not None:
            self.track.movement_history.append(observation.body_anchor)

    def _coast(self, *, arena_shape: Sequence[int], frame_index: int, occluded: bool) -> None:
        if self.track is None:
        ...
        self.track.last_seen_frame = min(self.track.last_seen_frame, frame_index)
    ...
    def coast(self, *, arena_shape: Sequence[int], frame_index: int, occluded: bool = True) -> TrackedSprite | None:
        self._coast(arena_shape=arena_shape, frame_index=frame_index, occluded=occluded)
    ...
    def update(
        ...
        *,
        ...
        scored: list[tuple[float, float, SpriteObservation, dict[str, object]]] = []
        for observation in observations:
            ...
            if observation.observation_id in role_conflicts:
                ...
                continue
            score, identity_score, diagnostic = self._candidate_score(
                observation,
                predicted=predicted,
            ...
            if score > 0.0:
                scored.append((score, identity_score, observation, diagnostic))
        scored.sort(key=lambda item: (item[0], item[1], item[2].body_confidence), reverse=True)
        selected: tuple[float, float, SpriteObservation, dict[str, object]] | None = None
        if scored:
            threshold = 0.42 if first_lock else self.config.self_association_min_score
            best = scored[0]
            ambiguous = len(scored) > 1 and best[0] - scored[1][0] < self.config.self_ambiguity_margin
            if best[0] >= threshold and not (ambiguous and not first_lock):
                selected = best
            elif ambiguous:
                self.ambiguity_count += 1
                self.last_rejection = "SELF_AMBIGUOUS_PRESERVED"
        if selected is None:
            self._coast(arena_shape=arena_shape, frame_index=frame_index, occluded=bool(excluded))
            self.last_observation_id = None
            self.last_association_score = 0.0
            self.last_identity_score = 0.0
            self.last_role_source = "predicted"
        ...
        if self.track is None:
            self.track = self._new_track(observation, frame_index)
            self.state = SelfTrackState.SELF_TRACKED
            self.last_role_source = "calibrated" if frame_index == 0 else "bootstrap"
        else:
            self._update_track(
                observation,
                frame_index=frame_index,
            ...
            self.state = SelfTrackState.SELF_TRACKED
            self.last_role_source = "tracked" if self.track.missing_frames == 0 else "reacquired"
        ...
        self.last_rejection = "-"
        self.ambiguity_count = 0
        self.identity.update(observation, clean=not observation.merged_body and score >= self.config.self_association_min_score)
