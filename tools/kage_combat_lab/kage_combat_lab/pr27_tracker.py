...
class SpriteTracker:
    ...
    def _self_conflict_observation(self, observation: SpriteObservation) -> str | None:
        ...
        if observation.body_bbox is not None and self.self_track.body_bbox is not None:
            overlap = bbox_iou(observation.body_bbox, self.self_track.body_bbox)
            if overlap >= 0.72:
                return "SELF_BBOX_OVERLAP_HARD_GATE"
        if observation.body_anchor is not None and self.self_track.body_anchor is not None:
            distance = math.hypot(
                observation.body_anchor[0] - self.self_track.body_anchor[0],
                observation.body_anchor[1] - self.self_track.body_anchor[1],
            )
            if distance <= max(8.0, self.config.merged_anchor_distance_px) and overlap if 'overlap' in locals() else False:
    ...
    def _association_score(
        ...
        *,
        ...
        if trainer_reason is not None and (target_track or track.effective_role is SpriteRole.ENEMY):
            return 0.0
    ...
    def _update_track(self, track: TrackedSprite, observation: SpriteObservation, frame_index: int, score: float) -> None:
        if track.effective_role is SpriteRole.SELF:
            self.role_flip_attempt = f"SELF->{observation.reserved_role.value}"
        ...
        track.identity_score = DescriptorFactory.similarity(track.appearance_signature, observation.descriptor)
    ...
    def _create_track(self, observation: SpriteObservation, frame_index: int) -> TrackedSprite | None:
        self_reason = self._self_conflict_observation(observation)
        if self_reason is not None:
            self.last_candidate_rejections.append(f"observation={observation.observation_id}:{self_reason}")
    ...
    def _self_anchor(self, arena_shape: Sequence[int]) -> tuple[float, float]:
        if self.self_track is not None and self.self_track.body_anchor is not None:
    ...
    def _classify_known_role(self, track: TrackedSprite, frame_index: int) -> None:
        trainer_reason = self._trainer_rejection_track(track)
        if trainer_reason is not None:
            lock_track_role(track, SpriteRole.NPC, frame_index=frame_index, source="trainer_exclusion", confidence=1.0)
        ...
        if known.category is SpriteClass.PLAYER:
            ...
            self.role_flip_blocked = "ROLE_FLIP_BLOCKED:NON_SELF_TRACK_CANNOT_BECOME_SELF"
            track.role_conflict_reason = self.role_flip_blocked
            track.classification = SpriteClass.UNKNOWN
            track.known_enemy = False
            return
        desired = SpriteRole.ENEMY if known.category is SpriteClass.ENEMY else SpriteRole.NPC
        if lock_track_role(track, desired, frame_index=frame_index, source="known_sprite", confidence=score):
    ...
    def _enemy_candidate_score(self, track: TrackedSprite, arena_shape: Sequence[int]) -> float:
        if track.track_state is not TrackState.TRACKED or not track.has_body_lock:
            return 0.0
        if track.effective_role not in {SpriteRole.UNKNOWN, SpriteRole.ENEMY}:
            return 0.0
        ...
        separation = math.hypot(track.body_anchor[0] - expected_self[0], track.body_anchor[1] - expected_self[1])
        if self.self_track is not None and bbox_iou(track.body_bbox, self.self_track.body_bbox) >= 0.72:
            self.last_candidate_rejections.append(f"track={track.track_id}:SELF_BBOX_OVERLAP_HARD_GATE")
            return 0.0
        if separation <= max(8.0, self.config.merged_anchor_distance_px):
            self.last_candidate_rejections.append(f"track={track.track_id}:SELF_ANCHOR_OVERLAP_HARD_GATE")
            return 0.0
        ...
        relative_proximity = max(
            0.0,
            1.0 - max(abs(track.anchor_cell[0]), abs(track.anchor_cell[1])) / (self.config.roi_radius_cells + 1.0),
        )
        close_bonus = max(0.0, 1.0 - separation / max(1.0, self.config.close_enemy_distance_px * 2.0))
        return (
            0.26 * persistence
            + 0.20 * shape
            + 0.13 * size
    ...
    def _select_context_enemy(self, arena_shape: Sequence[int], frame_index: int) -> None:
        ...
        scored = [(score, track) for score, track in scored if score >= 0.54]
        if not scored:
            self.pending_enemy_track_id = None
            self.pending_enemy_hits = 0
            return
        scored.sort(
            key=lambda item: (item[0], item[1].observations, item[1].body_confidence, -item[1].track_id),
            reverse=True,
        )
        ...
        if self.pending_enemy_hits < 2:
            return
        if lock_track_role(candidate, SpriteRole.ENEMY, frame_index=frame_index, source="context_enemy", confidence=max(score, 0.72)):
            candidate.confidence = max(candidate.confidence, score, 0.72)
            self.enemy_track_id = candidate.track_id
        self.pending_enemy_track_id = None
        self.pending_enemy_hits = 0

    def _close_candidate_score(self, observation: SpriteObservation, target: TrackedSprite) -> float:
        if observation.body_anchor is None or observation.body_bbox is None or self.self_track is None or self.self_track.body_anchor is None:
            return 0.0
        ...
        if distance_self > self.config.close_enemy_distance_px:
            return 0.0
        appearance = DescriptorFactory.similarity(target.appearance_signature, observation.descriptor)
        ...
        proximity_self = max(0.0, 1.0 - distance_self / self.config.close_enemy_distance_px)
        proximity_target = max(0.0, 1.0 - distance_target / (self.config.close_enemy_distance_px * 1.5))
        size = 0.0
        if target.body_bbox is not None:
            old_area = max(1, target.body_bbox[2] * target.body_bbox[3])
            new_area = max(1, observation.body_bbox[2] * observation.body_bbox[3])
            size = min(old_area, new_area) / max(old_area, new_area)
        return 0.42 * appearance + 0.25 * proximity_target + 0.18 * proximity_self + 0.10 * size + 0.05 * observation.body_confidence
    ...
    def _try_close_rebind(
        ...
        *,
        ...
        if target is None or target.track_state is TrackState.TRACKED:
            ...
            self.pending_rebind_hits = 0
            return unmatched, None
        scored = [(self._close_candidate_score(observation, target), observation) for observation in unmatched]
        scored = [(score, observation) for score, observation in scored if score >= 0.56]
        if not scored:
            self.pending_rebind_key = None
            self.pending_rebind_hits = 0
            return unmatched, None
        scored.sort(key=lambda item: (item[0], item[1].body_confidence), reverse=True)
        score, candidate = scored[0]
        ...
        if key == self.pending_rebind_key:
            ...
            self.pending_rebind_hits = 1
        if self.pending_rebind_hits < self.config.close_reacquire_confirm_frames:
            return unmatched, None
        ...
        self.pending_rebind_key = None
        self.pending_rebind_hits = 0
        return [observation for observation in unmatched if observation.observation_id != candidate.observation_id], target.track_id
    ...
    def update(
        ...
        *,
        ...
        self.player_track_id = None if self_track is None else self_track.track_id
        ...
        for observation in observations:
            if observation.observation_id in reserved:
                self.shared_observation_blocked = True
                self.last_candidate_rejections.append(f"observation={observation.observation_id}:SELF_OBSERVATION_RESERVED")
                continue
            if observation.observation_id in merged or observation.merged_body:
                self.last_candidate_rejections.append(f"observation={observation.observation_id}:MERGED_BODY_EXCLUDED")
                continue
        ...
        for track in active_tracks:
            ...
            for observation_index, observation in enumerate(allowed_observations):
                ...
                self.association_diagnostics.append({
                    ...
                    "position_score": None,
                    "appearance_score": DescriptorFactory.similarity(track.appearance_signature, observation.descriptor),
                    "size_score": None,
                    "motion_score": None,
                    "role_gate_passed": score > 0.0,
                    "rejection_reason": None if score > 0.0 else "ASSOCIATION_GATE",
                })
                if score >= threshold:
        ...
        for _, score, track_id, observation_index in candidates:
            if track_id in used_tracks or observation_index in used_observations:
                continue
            self._update_track(self.tracks[track_id], allowed_observations[observation_index], frame_index, score)
        ...
        unmatched, rebound_track_id = self._try_close_rebind(unmatched, frame_index=frame_index)
        ...
        if target_track is not None and target_track.anchor_cell is not None and target_track.track_state is TrackState.TRACKED:
            unmatched = [
                observation
                for observation in unmatched
                if observation.anchor_cell is not None
                and (
                    (observation.anchor_cell[0] - target_track.anchor_cell[0]) ** 2
                    + (observation.anchor_cell[1] - target_track.anchor_cell[1]) ** 2
                    <= self.config.target_focus_radius_cells ** 2
                    or observation.anchor_cell[0] ** 2 + observation.anchor_cell[1] ** 2 <= 4
                )
            ]
        ...
        if self_track is not None and self_track.body_anchor is not None:
            for observation in allowed_observations:
                if observation.body_anchor is None:
                    continue
                distance = math.hypot(
                    observation.body_anchor[0] - self_track.body_anchor[0],
                    observation.body_anchor[1] - self_track.body_anchor[1],
                )
                if distance <= self.config.close_enemy_distance_px and self._self_conflict_observation(observation) is None:
