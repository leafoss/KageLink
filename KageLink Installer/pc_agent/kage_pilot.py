...
def build_dojo_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        description=(
            "Kage Pilot Dojo training through the canonical KageLink 3.5 runtime / "
            "treinamento no Dojo pelo runtime canonico do KageLink 3.5"
        ),
    )
    parser.add_argument(
        "--config",
...
# Stable names for tests and integrations that used the old duplicate entry point.
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not values:
        print("Kage Pilot / KageLink 3.5")
        print("Main command / Comando principal: python kage_pilot.py dojo")
        print("Use --help after a command for details / use --help apos um comando")
        return 0
    ...
    print(
        "Use: dojo, record, mark, train, train-v2, pilot, pilot-v2, "
