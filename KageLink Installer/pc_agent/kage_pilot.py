...
def build_dojo_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        description=(
            "Kage Pilot Dojo training using the physically validated canonical engine / "
            "treinamento no Dojo usando o motor canonico validado fisicamente"
        ),
    )
    parser.add_argument(
        "--config",
...
# Stable aliases for tests/integrations that previously imported kage_pilot_dojo.
...
def main(argv: list[str] | None = None) -> int:
    ...
    print("Use: dojo, record, mark, train, train-v2, pilot, pilot-v2, capture-template, init-config, dojo-v2", file=sys.stderr)
    return 2
