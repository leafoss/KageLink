...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Kage Pilot v0.3e bounded Dojo loop: request -> fight -> recover -> repeat / "
            "loop limitado: solicitar -> lutar -> recuperar -> repetir"
        )
    )
    parser.add_argument(
        "--rounds",
        type=int,
        default=1,
        help="Number of fights. Use 0 to continue until F12 / Numero de lutas; 0 ate F12.",
    )
...
def _run_round(args, *, round_number: int) -> bool:
    script = Path(__file__).with_name("kage_pilot_live_v03e_round.py")
...
def main() -> int:
    ...
    print("Kage Pilot v0.3e FULL DOJO LOOP")
    print("READY -> CLICK TRAINER -> WAIT 10s -> ENTER -> WAIT 5s -> COMBAT -> RECOVER -> REPEAT")
    print(f"rounds={'until F12' if rounds == 0 else rounds} / rodadas={'ate F12' if rounds == 0 else rounds}")
    print("START ADJACENT TO TRAINER, RECOVERED, NOT MEDITATING")
    ...
    while rounds == 0 or round_number <= rounds:
        ...
        print(f"ROUND {round_number}: REQUEST TAIJUTSU DOJO SPAR")
        try:
