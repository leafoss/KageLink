...
def test_target_below_sends_down_pulse_immediately_before_h() -> None:
    ...
    actions = physical.execute(decision(face="DOWN"))

    down_index = controller.calls.index(("down", "r"))
    h_index = controller.calls.index(("h", "r"))
    assert down_index < h_index
    assert actions == ("AIM_DOWN_50MS", "H_50MS", "OBSERVE_150MS")
    assert sleeps == [0.05, 0.05, 0.15]


def test_h_is_rejected_when_current_frame_has_no_direction() -> None:
    controller = FakeController()
    physical = PhysicalCombatInput(controller, sleep_fn=lambda _: None)
    physical.activate()
    physical.start_combat_hold()

    with pytest.raises(RuntimeError, match="H_REQUIRES_CARDINAL_AIM"):
        physical.execute(decision(face=None))
