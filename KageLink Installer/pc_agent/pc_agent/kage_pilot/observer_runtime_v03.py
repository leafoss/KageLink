...
from .entity_observer import EntityObserver, ObserverConfig, ObserverState
...
class StableTargetObserver(EntityObserver):
    """EntityObserver with persistent target lock hysteresis."""
...
def render_overlay_v03(
    ...
    """Render v0.3 diagnostics with a tight vertical PLAYER box, not a circle."""
    ...
    lines = [
        ...
        f"target acquire: {config.target_acquire_threshold:.0f}%",
        f"target keep: {config.target_keep_threshold:.0f}%",
        "",
    ]
    ...
    if state.motion_mask is not None and state.motion_mask.size:
        inset_width = min(panel_width - 24, 340)
