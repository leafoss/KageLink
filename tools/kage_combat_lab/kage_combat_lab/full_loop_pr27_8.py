...
def _capture_pre_spawn_scene(path: Path) -> None:
    ...
    try:
        ...
        if not cv2.imwrite(str(path), native.bgr):
            raise RuntimeError(f"PR27_8_PRE_SPAWN_WRITE_FAILED:{path}")
        print(
            f"PR27_8_PRE_SPAWN_SCENE_READY path={path.resolve()} "
...
def main() -> int:
    ...
    if int(args.rounds) != 1:
        print("PR27_8_ONE_PERCEPTION_ROUND_ONLY")
        return 2

    print("KAGE COMBAT LAB - PR27.8 FIXED GRID PERCEPTION LOOP")
    print("OUTER FLOW: Trainer -> dialog OK -> PRE_SPAWN_SCENE -> wait spawn -> perception")
    ...
    if bbox is not None:
        ...
        if len(values) == 4:
            os.environ["KAGE_PR278_TRAINER_BBOX_OUTPUT"] = ",".join(
                str(value) for value in values
            )
            print(f"PR27_8_TRAINER_BBOX_OUTPUT={values}")
