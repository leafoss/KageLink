...
class CameraMotionEstimator:
    """Estimate dominant terrain translation using only trusted static regions.

    PR27.9 adds a strict stationary-perception mode. When no directional input is
    sent, an apparent shift above 2 px is evidence of a bad match, not authority to
    warp the frame. Rejected shifts are reported but never applied.
    """
    ...
    def _prepare(
        ...
        static_terrain_mask: np.ndarray | None,
    ) -> np.ndarray:
    ...
    def _flow_shift(previous: np.ndarray, current: np.ndarray) -> tuple[float, float, float, int]:
        ...
        consensus = float(inlier_count / len(vectors))
        return float(refined[0]), float(refined[1]), consensus, valid_count
    ...
    def observe(
        ...
        *,
        ...
        self.last = CameraMotionResult(
            ...
            flow_consensus=float(flow_consensus),
            phase_flow_disagreement=disagreement,
            stationary_mode=bool(stationary_mode),
            shift_accepted=bool(accepted),
            shift_rejection_reason=rejection,
